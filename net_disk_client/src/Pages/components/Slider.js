  ...
  useEffect(() => {
    ...
    if (
      ...
      axios({
        ...
        cookie.save("email", res.data.data.email, {
          expires: cookieTime,
          path: "/",
        });
        cookie.save("user_name", res.data.data.name, {
          expires: cookieTime,
          path: "/",
        });
        cookie.save("telephone", res.data.data.telephone, {
          expires: cookieTime,
          path: "/",
        });
        cookie.save("signature", res.data.data.signature, {
          expires: cookieTime,
          path: "/",
        });
      ...
      <Sider width={"18vw"} className="sider">
        ...
        >
          <Menu.Item key="usage" style={{ height: "7vh" }}>
            <div>
              <PieChartOutlined style={{ fontSize: "3vh", color: "#108ee9" }} />
              <span className="span2">使用情况</span>
            </div>
          </Menu.Item>
          {/* <span className='span1'>所有文件</span> */}
          <Menu.Item key="all" style={{ height: "7vh" }}>
            <div>
              <DesktopOutlined style={{ fontSize: "3vh", color: "#108ee9" }} />
              <span className="span2">全部文件</span>
            </div>
          </Menu.Item>
          <Menu.Item key="recycle_bin" style={{ height: "7vh" }}>
            <div>
              <CloudUploadOutlined
                style={{ fontSize: "3vh", color: "#108ee9" }}
              />
          ...
          <Menu.Item key="aboutus" style={{ height: "7vh" }}>
            <div>
              <QuestionCircleOutlined
                style={{ fontSize: "3vh", color: "#108ee9" }}
              />
              <span className="span2">关于我们</span>
            </div>
          </Menu.Item>
        </Menu>
